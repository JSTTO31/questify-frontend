<template>
  <v-card
    class="pa-0"
    style="user-select: none; transition: 0.15s linear"
    :class="isDragover ? 'bg-grey-lighten-1' : ''"
    @drop="($event) => drop($event, question.group_name)"
    @dragover="dragOver"
  ...
  >
    <v-list-item
      ...
      @contextmenu.prevent="($event: any) => emits('right-click', $event, question)"
      color="primary"
      :active="
        selectedMenuDialog.length > 0 &&
        //@ts-ignore
        selectedMenuDialog[0].group_name == question.group_name
      "
      :title="question.group_name"
    >
      <template #prepend>
    ...
    <Transition name="slide-down">
      <v-list class="pa-0" v-if="showLists">
        <v-list-item
          class="pl-15 pa-0"
          v-for="(child, n) in question.questions"
          :key="child.index"
          style="user-select: none"
          :color="
            //@ts-ignore
            isChildSelected(child) && !selectedMenuDialog[0].group_name
              ? 'blue'
              : '#FF9B82'
          "
          :active="
            selectedQuestion.index == child.index ||
            //@ts-ignore
            (isChildSelected(child) && !selectedMenuDialog[0].group_name)
          "
...
const emits = defineEmits([
  "right-click",
  "update:showMenuDialog",
  "update:selectedMenuDialog",
]);
const props = defineProps<{
  selectedMenuDialog: Question[];
  question: { group_name: string; questions: Question[] };
  index: number;
}>();
const { selectedQuestion, questionnaire } = storeToRefs(useQuestionnaireStore());
const isDragover = ref(false);
const someIsSelected = computed(() =>
  props.question.questions.some((item) =>
    props.selectedMenuDialog.some((innerItem) => innerItem.index == item.index)
  )
);
const isChildSelected = computed(() => (child: Question) =>
  props.selectedMenuDialog.some((selected) => selected.index == child.index)
);
