<template>
  <v-app>
    ...
    <v-app-bar height="35" flat class="text-white text-h6 pa-10 px-15 font-weight-regular" color="transparent" style="z-index: 100;" v-if="route.name  == 'response.question'">
      <v-spacer></v-spacer>
    ...
    <span style="position: fixed;top: 30px;left: 40px;z-index: 100;" v-if="$route.name == 'response.question'">
      <v-card class="rounded-circle" color="transparent" style="user-select: none;" @click="showQuestionList = true">
        <v-progress-circular color="primary" :model-value="(currentIndex + 1) / questionnaire.questions.length * 100" size="150" width="8" class="text-h5 font-weight-medium">{{ currentIndex + 1 }}/{{ questionnaire.questions.length }}</v-progress-circular>
      </v-card>
    </span>
    <span style="position: fixed;bottom: 50px;right: 80px;" v-if="$route.name == 'response.question'">
      <v-btn class="ml-5 rounded-circle" :size="90" @click="currentResponseQuestion.marked = true" v-if="currentResponseQuestion && !currentResponseQuestion.marked">
        <v-icon size="40">mdi-lightbulb-outline</v-icon>
      </v-btn>
      <v-btn class="ml-5 rounded-circle" color="primary" :size="90" @click="currentResponseQuestion.marked = false" v-if="currentResponseQuestion && currentResponseQuestion.marked">
    ...
    <div v-if="$route.name == 'response.question'" class="d-flex pb-5" style="padding-inline: 485px;position: fixed;bottom: 0;left: 0;width: 100%;z-index: 50000;">
      ...
      <div class="d-flex" >
        <v-btn color="primary" size="x-large" class="ml-4" @click="showSubmit = true" v-if="last_question.id == question.id || response.reviewed">Submit</v-btn>
