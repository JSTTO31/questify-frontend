<template>
  <v-list class="pa-0">
    <div v-if="currentResponse && currentResponse.answer_keys.length < 1">
      <v-alert variant="tonal" color="error" class="pa-4" icon="mdi-close"
        >No Selected Item</v-alert
      >
    </div>
    <div v-else>
      <v-alert
        ...
        "
        class="pa-4 mb-2"
        v-for="answer in currentResponse.answer_keys"
        :key="answer"
      >
        {{ answer }}
      </v-alert>
...
const currentResponse = computed(() => {
  const currentResponse: any = response.value.question_responses.find(
    (item) => item.question_id == props.question.id
  );
...
const answerCovertText = (index: number) => {
  return props.question.answers.find((answer) => answer.index == index)?.text || "";
};
const status = (answer: string) => {
  return !getRightAnswers.value
    ? "pending"
    : getRightAnswers.value.some((item) => answerCovertText(parseInt(item)) == answer)
    ? "correct"
    : "wrong";
};
</script>
